...
OtherNode::OtherNode(
  ...
  append_entries_ = rclcpp::Client<fsros_msgs::srv::AppendEntries>::make_shared(
  ...
  request_vote_ = rclcpp::Client<fsros_msgs::srv::RequestVote>::make_shared(
      node_base.get(), node_graph,
      NodeUtil::get_service_name(cluster_name, node_id,
...
bool OtherNode::broadcast(uint64_t current_term, uint32_t node_id,
  ...
  auto response = append_entries_->async_send_request(
      request,
      [=](rclcpp::Client<
          fsros_msgs::srv::AppendEntries>::SharedFutureWithRequest future) {
        auto ret = future.get();
